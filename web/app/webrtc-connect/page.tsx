...
function WebRTCConnectPageContent() {
  ...
  const connectionRef = useRef<SoraSendRecvConnection | null>(null);
  ...
  async function connectToChannel() {
    ...
    try {
      ...
      const sendrecv = connection.sendrecv(channelID) as SoraSendRecvConnection;
      ...
      sendrecv.on("connected", () => {
        setStatus("接続中");
        appendLog("Sora への接続が確立しました。");
      });
      ...
      connectionRef.current = sendrecv;
      setHasActiveConnection(true);
      await sendrecv.connect(mediaStream);
      appendLog("接続処理が完了しました。");
    } catch {
      connectionRef.current = null;
      resetStreams();
      setStatus("未接続");
      setHasActiveConnection(false);
      setErrorMessage(
        "WebRTC 接続に失敗しました。カメラ・マイク権限とチャンネル情報を確認してください。",
      );
      appendLog("接続に失敗しました。");
    } finally {
      setIsConnecting(false);
  ...
  return (
    <main>
      <h1>WebRTC 双方向接続（β）</h1>
      <p className="mx-auto">
        作成済みの WebRTC チャンネルへ双方向接続します。
      </p>
...
export default function WebRTCConnectPage() {
  return (
    <Suspense
      fallback={
        <main>
