...
type CreateRTMPChannelResponse = {
  channel_id: string;
  ingest_url: string;
  playlist_url: string;
};
...
export default function RTMPToHLSPage() {
  ...
  const [createdChannel, setCreatedChannel] =
    useState<CreateRTMPChannelResponse | null>(null);
  ...
  async function createRTMPChannel() {
    const trimmedToken = token.trim();
    if (trimmedToken === "") {
      setSuccessMessage("");
      setErrorMessage("APIトークンを入力してください。");
    ...
    try {
      const response = await fetch(
        `${apiBaseURL}/api/imageflux/channels/rtmp-to-hls`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${trimmedToken}`,
      ...
      const data = (await response.json()) as CreateRTMPChannelResponse;
      setCreatedChannel(data);
      setSuccessMessage("RTMP to HLSチャンネルを作成しました。");
    } catch (error) {
  ...
  return (
    <main>
      <h1>RTMP to HLS</h1>
      <p className="mx-auto">
        APIトークンと変換設定を入力して、RTMP to HLSチャンネルを作成できます。
      </p>
      <section>
      ...
      {createdChannel !== null ? (
        <dialog
          open
          aria-labelledby="rtmp-result-dialog-title"
          onCancel={(event) => {
                ...
                <table>
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>RTMP配信先URL</td>
                      <td>
                        <span>{createdChannel.ingest_url || "-"}</span>
                        <button
                          type="button"
                          onClick={() =>
                            copyToClipboardWithMessage(
                              "RTMP配信先URL",
                              createdChannel.ingest_url,
                              setCopyMessage,
                            )
                          }
                          className="rounded-md p-1 transition bg-stone-100 hover:bg-stone-600"
                          aria-label="RTMP配信先URLをコピー"
                          title="RTMP配信先URLをコピー"
                        >
                          <Image
                            className="copy_button"
                            src="/copy-svgrepo-com.svg"
                    ...
                    <tr>
                      <td>HLSプレイリストURL</td>
                      <td>
                        <span>{createdChannel.playlist_url || "-"}</span>
                        <button
                          type="button"
                          onClick={() =>
                            copyToClipboardWithMessage(
                              "HLSプレイリストURL",
                              createdChannel.playlist_url,
                              setCopyMessage,
                            )
                          }
                          className="rounded-md p-1 transition bg-stone-100 hover:bg-stone-600"
                          aria-label="HLSプレイリストURLをコピー"
                          title="HLSプレイリストURLをコピー"
                        >
                          <Image
                            className="copy_button"
                            src="/copy-svgrepo-com.svg"
